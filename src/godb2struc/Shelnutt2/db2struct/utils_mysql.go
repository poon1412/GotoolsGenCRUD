...
func generateMysqlTypes(obj map[string]map[string]string, depth int, jsonAnnotation bool, gormAnnotation bool, gureguTypes bool) (string, string) {
	structure := "struct {"
	...
	for _, key := range keys {
		...
		if strings.Contains(valueType, "null") && gureguTypes {
			hasNull = true
		}
	...
	if hasTime || hasSql || hasNull{
		importStr = "import(\n"
		if hasSql {
			importStr =  importStr + "\"database/sql\"\n"
		}
		if hasTime {
			importStr =  importStr + "\"time\"\n"
		}
		if hasNull {
			importStr =  importStr + "\"gopkg.in/guregu/null.v3\"\n"
		}
		importStr = importStr +  ")"
	}

	return structure, importStr
}
