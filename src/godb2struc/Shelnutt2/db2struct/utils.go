...
func Generate(columnTypes map[string]map[string]string, tableName string, structName string, pkgName string, jsonAnnotation bool, gormAnnotation bool, gureguTypes bool) ([]byte, error) {
	...
	dbTypes, importStr = generateMysqlTypes(columnTypes, 0, jsonAnnotation, gormAnnotation, gureguTypes)
 

	src := fmt.Sprintf("package %s\n%s\n  type %s %s}",
		pkgName,
		importStr,
		fmtFieldName(stringifyFirstChar(structName)),
		dbTypes)
	if gormAnnotation == true {
		tableNameFunc := "// TableName sets the insert table name for this struct type\n" +
			"func (" + strings.ToLower(string(structName[0])) + " *" + fmtFieldName(stringifyFirstChar(structName)) + ") TableName() string {\n" +
			"	return \"" + tableName + "\"" +
			"}"
		src = fmt.Sprintf("%s\n%s", src, tableNameFunc)
	}
